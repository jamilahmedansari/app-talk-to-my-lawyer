...
async function handleRoute(request, { params }) {
  ...
  try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const user = await tx.user.create({
          data: {
            ...
            subscriptionLettersRemaining: 0,
            isActive: true
          }
        })
      ...
      return handleCORS(NextResponse.json({ 
        user: { 
          ...
          subscription: {
            ...
            lettersRemaining: result.subscriptionLettersRemaining
          }
        },
        token,
    ...
    if (route === '/auth/register-with-coupon' && method === 'POST') {
      ...
      // Create user with 20% discount using transaction
      ...
      const result = await prisma.$transaction(async (tx) => {
        const user = await tx.user.create({
          data: {
            ...
            subscriptionReferredBy: contractor.userId,
            isActive: true
          }
        })
      ...
      return handleCORS(NextResponse.json({ 
        user: { 
          ...
          subscription: {
            status: result.subscriptionStatus,
            discount_percent: result.subscriptionDiscountPercent,
            referred_by: result.subscriptionReferredBy
          }
        },
        token,
      ...
      return handleCORS(NextResponse.json({ 
        user: { 
          ...
          subscription: {
            ...
            lettersRemaining: user.subscriptionLettersRemaining || 0,
            packageType: user.subscriptionPackageType,
          ...
          case 'checkout.session.completed':
            ...
            const updateResult = await prisma.user.update({
              ...
              data: {
                ...
                subscriptionPlanId: session.payment_intent,
                subscriptionPackageType: packageType,
                subscriptionLettersRemaining: lettersCount,
    ...
    if (route === '/documents/generate' && method === 'POST') {
      ...
      if (user.subscriptionStatus !== 'paid' || user.subscriptionLettersRemaining <= 0) {
        return handleCORS(NextResponse.json({ 
      ...
      try {
        ...
        // Use transaction to save document and update user
        const result = await prisma.$transaction(async (tx) => {
          ...
          await tx.user.update({
            where: { id: decoded.userId },
            data: { 
              subscriptionLettersRemaining: { decrement: 1 }
            }
          })

          return { document, lettersRemaining: user.subscriptionLettersRemaining - 1 }
        })
    ...
    if (route === '/letters/generate' && method === 'POST') {
      ...
      try {
        ...
        // Use transaction to save letter and update user
        const result = await prisma.$transaction(async (tx) => {
          ...
          return { letter, lettersRemaining: user.subscriptionLettersRemaining - 1 }
        })
    ...
    if (route.startsWith('/letters/') && !route.includes('/stage') && !route.includes('/send') && method === 'GET') {
      ...
      return handleCORS(NextResponse.json({ letter: letter }))
    }
    ...
    if (route === '/letters' && method === 'GET') {
      ...
      return handleCORS(NextResponse.json({ letters }))
    }
    ...
    if (route === '/remote-employee/stats' && method === 'GET') {
      ...
      return handleCORS(NextResponse.json({
        ...
        total_signups: contractor.totalSignups,
    ...
    if (route === '/contractor/stats' && method === 'GET') {
      ...
      const totalCoupons = contractor.coupons.length
      const activeCoupons = contractor.coupons.filter(c => c.expiresAt > new Date() && c.currentUses < c.maxUses).length
    ...
    if (route === '/admin/users' && method === 'GET') {
      ...
      const users = await prisma.user.findMany({
        ...
        select: {
          ...
          subscriptionPackageType: true,
          subscriptionLettersRemaining: true,
          isActive: true,
          createdAt: true,
          updatedAt: true
        }
